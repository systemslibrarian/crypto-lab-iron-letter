 ...
 *    files the node under `incomplete`. The modal's backdrop is `bg-black/60`
 *    plus a `backdrop-blur`, and the hero's "why it matters" card is a
 *    `color-mix(...)` amber wash over the page, so their text composites onto
 *    whatever is behind rather than onto a flat colour axe can read.
 ...
 *    `opacity: .85`, and `.ecdh-secret` — the 32 bytes the whole ECDH exhibit
 *    exists to show — animates in from `opacity: .25`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Every key pane
 *    here is `max-h-24` (the ciphertext `max-h-32`) with `overflow-y: auto`, and
 *    an RSA-4096 private key is several times that height, so most of it sits
 *    outside its scroller's client box at any moment. Content scrolled past that
 ...
 *    through to white, inventing a failure. Skip it, and rely on the part that
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. The only
 *    SVG this lab draws is the QR code, which is `<rect>`s and no text, so this
 *    rule finds nothing here — it is kept because it costs one branch and is
 *    silent when unused. Where it matters: an `<svg>` draws a shape as a `<rect>`
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" card is `color-mix(in srgb, ...)`, the shared header's button
     * borders are another, and Tailwind's slash-opacity utilities (`bg-black/60`
     * on the modal backdrop, `border-zinc-800/50` on the table rows) all resolve
     * through `color-mix()` too — and Chromium reports those to
     ...
     * failures (dark-on-light read as dark-on-dark) and, far worse, could hide a
     ...
     *
     * This page renders an RSA-4096 private key as one long base64url text node
     * inside four nested cards, a three-column comparison table, and a
     * `<dl>` per MITM step, and every one of them walks the same handful of
     * ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's disclosure is the "Reveal private key"
      // <details> in each algorithm tab, whose open state is a real state the
      // gate reaches by clicking the summary and scans explicitly.
